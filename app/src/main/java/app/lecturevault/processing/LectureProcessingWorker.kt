...
class LectureProcessingWorker(
    ...
    private suspend fun tryCloud(sessionId: String): CloudOutput? {
        if (!settings.consent) return null
        val groqKey = secretStore.getGroqKey() ?: return null
        val geminiKey = secretStore.getGeminiKey() ?: return null
        return try {
            val session = checkNotNull(repository.get(sessionId))
            ...
            CloudOutput(CombinedTranscript(text, timestamps), summary)
        } catch (cancelled: CancellationException) {
            throw cancelled
        } catch (_: Exception) {
