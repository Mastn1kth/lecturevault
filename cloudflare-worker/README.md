...
- `TRUSTED_PROXY_TOKEN`: optional long random secret for the separate Docker gateway proxy. It lets the Worker safely use the visitor's forwarded IP for per-user limits.

Public endpoint: `https://api.gory-staff.ru`. Available routes: `GET /health`, `POST /v1/transcribe` (multipart `file`), `POST /v1/generate` (`summary` or `quiz`).

The Worker applies a per-IP daily limit of 12 audio uploads and 30 text requests using a Durable Object. Do not put provider keys or `TRUSTED_PROXY_TOKEN` in `wrangler.jsonc`, Git, APK, IPA or JAR.
